...
def feature_extraction(path, df, sampling_rate=48000):
    ...
    # Jitter (approximation: std(f0)/mean(f0))
    ...
    # HNR (approximate using harmonic ratio)
    y_harm, y_perc = librosa.effects.hpss(audio)
    hnr = np.mean(librosa.feature.rms(y=y_harm)) / (np.mean(librosa.feature.rms(y=y_perc)) + 1e-6)
...
def create_header():
    header = [
        'filename', 'gender', 'spectral_centroid', 'spectral_bandwidth',
        'spectral_rolloff', 'zero_crossing_rate', 'rms_energy', 'f0_mean', 'f0_std',
        'jitter', 'shimmer', 'hnr'
    ]
    header += [f'mfcc{i}' for i in range(1, 21)]
    header.append('age')
    return header
