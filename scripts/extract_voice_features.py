...
# Another code to extract Gender, using "male", "female", and "unknown"
# def get_gender(df, path):
#     file_name = path.split("/")[-1].replace(".mp3", "")
#     gender_row = df[df['path'].str.contains(file_name)]
#     if gender_row.empty:
#         return "unknown"
#     gender_value = gender_row.iloc[0]['gender']
#     return gender_value if gender_value in ['male', 'female'] else "unknown"
...
def create_header():
    # Define header
    header = ['filename', 'gender', 'spectral_centroid', 'spectral_bandwidth', 'spectral_rolloff', 'zcr', 'rms'] + \
...
def create_feature_csv(tsv_file):
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            row_out = [row["filename"]] + list(features) + [row["age"]]
